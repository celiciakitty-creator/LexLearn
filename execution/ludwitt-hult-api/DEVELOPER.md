...
The reference API stores data in **Supabase Postgres**. **Redeploying the API preserves metrics** as long as `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` point to the same project.
...
# Set SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, HULT_* in .env
...
Run the SQL migration via Supabase CLI — see [DEPLOY.md](DEPLOY.md):
...
Developer keys seeded on startup (idempotent):



- `sandbox_key_demo` — sandbox (events not counted)

- `HULT_DEV_API_KEY` if set, otherwise `prod_key_demo` — production counting



Set `HULT_DEVELOPER_HANDLE` to your GitHub username in production so self-testing is blocked from qualified counts.
...
Cohort member user IDs do not count. User IDs that **exactly match** your developer handle do not count. Email dedupe is described in cohort docs but is **not implemented** in this reference API. The roster blocklist ships with placeholder IDs.
...
Returns CSV: `app_id, student_handle, unique_users, qualified_users`. Set `ADMIN_KEY` in production.
